...
const ColorInput: FC<ColorInputProps> = ({ value, onChange, label, size = 'md' }) => {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className={`flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-[0.625rem] ${sizeClasses.container}`}>
        <div className={`relative ${sizeClasses.swatch} mr-3 rounded-md border overflow-hidden transition-all ${
          isValid 
...
export const ColorTab: FC<ColorTabProps> = ({ state, updateState, updateGradientStop }) => {
  ...
  const addGradientStop = () => {
    ...
    if (newStops.length === 0) {
      ...
      const midPosition = Math.round((secondLastStop.position + lastStop.position) / 2);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {state.colorMode === 'solid' ? (
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 text-[10px] font-medium bg-indigo-500 hover:bg-indigo-600 text-white rounded-md transition-colors"
              ...
              {state.gradientStops.map((stop, index) => (
                ...
                >
                  <span className="w-10 text-[9px] font-bold text-zinc-400 uppercase tracking-wider flex-shrink-0">
                    {index === 0 ? 'Start' : index === state.gradientStops.length - 1 ? 'End' : `Stop ${index}`}
                  </span>
          ...
          {/* Gradient Preview */}
          <div className="space-y-2">
            <p className="text-xs font-medium text-zinc-700 dark:text-zinc-300 px-1">Preview</p>
            <div 
